...
def update_svn_changed_files(svn, changeset, ws_root,
    ...
    paths = filter(lambda p: p['path'].startswith(project_dir), paths)
...
def update_p4d_changed_files(p4d, ws_root, changelist, depot_path_map):
    ...
    depotFiles = filter(None, depotFiles)
    depotFiles = map(translate_p4_filename, depotFiles)

    p4_paths = map(depot_path_map.translate, depotFiles)
...
def verify_changes(src_svn, dst_p4d, src_root, dst_root,
    ...
    for dst_change in dst_changes_iter:
        ...
        dst_files, dst_msg = update_p4d_changed_files(dst_p4d, dst_root, dst_change,
                                                      dst_depot_abspath_map)
        ...
        while ((not new_src_file_hash) or
            ...
            src_change = src_changes_iter.next()
            logger.info('against svn change %s' % src_change)

            src_files, src_msg = update_svn_changed_files(src_svn,
                                                          src_change,
                                                          src_root,
                                                          svn_ignore_externals=svn_ignore_externals)

            if svn_project_subdir:
                logger.info('svn_project_subdir: %s' % svn_project_subdir)
                svn_project_subdir_path = os.path.join(src_root, svn_project_subdir[1:])
                new_src_file_hash = get_dir_file_hash(svn_project_subdir_path,
                                                      exclude=excluded_subdirs,
                                                      detect_dir=False,
                                                      excluded_files=excluded_files)
            else:
                new_src_file_hash = get_dir_file_hash(src_root,
                                                      exclude=excluded_subdirs,
                                                      detect_dir=False,
                                                      excluded_files=excluded_files)
...
def get_svn_rev_list(svn_docker_cli, svn_dir):
    ...
    svn = SvnPython('svn://%s:3690/repos' % svn_ip, svn_user, svn_pass, svn_dir)
...
def verify_replication(src_mapping, dst_mapping,
    ...
    try:
        ...
        pass
    dst_ws_mapping = map(P4Server.WorkspaceMapping._make, dst_mapping)
...
def get_svn_from_docker(docker_cli):
    ...
    svn = SvnPython('svn://%s:3690/repos' % svn_ip, svn_user, svn_pass, svn_root)
    ...
    with get_svn_from_docker(docker_cli) as (svn, svn_root):
        ...
        for action in actions:
            ...
            if action in ['symlink_rel']:
                ...
                test_file_rel_path = os.path.join('./', os.path.basename(test_file))
                act_str = '%s %s %s' % (action, test_file_rel_path, test_symlink_file)
