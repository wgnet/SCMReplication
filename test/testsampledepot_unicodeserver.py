...
from testcommon import set_p4d_unicode_mode, replicate_sample_view
...
class P4P4ReplicationTestUnicodeServer(ReplicationTestCaseWithDocker):
    def replicate_sample_dir_withdocker_unicodeserver(
            self, depot_dir, **kwargs):
...
class SampleDepotTestSourceUnicodeServer(
        P4P4ReplicationTestUnicodeServer,
    ...
    def replicate_sample_dir_withdocker(self, depot_dir, **kwargs):
        return self.replicate_sample_dir_withdocker_unicodeserver(
            depot_dir, src_unicode_server=True, **kwargs)

    def test_replicate_sample_commit_message_reformat_review(self):
        pass


class SampleDepotTestTargetUnicodeServer(
        P4P4ReplicationTestUnicodeServer,
        testsampledepot_misc.SampleDepotTestMisc):
    @classmethod
    def setUpClass(cls):
    ...
    def replicate_sample_dir_withdocker(self, depot_dir, **kwargs):
        return self.replicate_sample_dir_withdocker_unicodeserver(
            depot_dir, dst_unicode_server=True, **kwargs)

    def test_replicate_sample_commit_message_reformat_review(self):
        pass


class SampleDepotTestBothUnicodeServer(
        P4P4ReplicationTestUnicodeServer,
        testsampledepot_misc.SampleDepotTestMisc):
    @classmethod
    def setUpClass(cls):
    ...
    def replicate_sample_dir_withdocker(self, depot_dir, **kwargs):
        return self.replicate_sample_dir_withdocker_unicodeserver(
            depot_dir, src_unicode_server=True, dst_unicode_server=True, **kwargs)
